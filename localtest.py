...
t2 = Transformation('stir', [t1.output], 'batter')
t3 = Transformation('boil', [t2.output], 'thick batter')
t4 = Transformation('mix', [t3.output, vanilla, cereal], 'textured batter')
t5 = Transformation('chill', [t4.output], 'cookies')
t1a = Transformation('stir', [cream], 'whipped cream')
t6 = Transformation('mix', [t5.output, t1a.output], 'decorated cookies')
...
while recipe.active_nodes:
    node = random.choice(recipe.active_nodes)
    print(node.step)
    recipe.execute(node.transformation)
